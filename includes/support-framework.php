 ...
 * @since 1.0
 */
function uplifted_add_theme_support_framework() {
	...
	add_theme_support( 'ctfw-after-activation', array(
		...
		'notice'				=> sprintf( __( '<b>Next Steps:</b> Please continue reading the <a href="%s" target="_blank">Getting Started</a> guide for the next steps after theme activation.', 'uplifted' ), 'https://churchthemes.net/getting-started-uplifted/' ),
	...
	// Page templates must be specified with ctfw_content_types filter. See uplifted_update_content_types().
	...
	add_theme_support( 'ctfw-gallery-thumb-size', array(
		'3' => 'uplifted-rect-large', 		// when 1 to 3 columns used
		'5' => 'uplifted-rect-medium', 	// when 4 to 5 columns used
		'9' => 'uplifted-rect-small',  	// when 6 to 9 columns used
	) );
...
add_action( 'after_setup_theme', 'uplifted_add_theme_support_framework' );
 ...
 * @since 1.0
 */
function uplifted_add_theme_support_framework_widgets() {
	...
	add_theme_support( 'ctfw-widget-archives', array(
		...
		'field_overrides' => array(
			'post_type' => array( // explicitly set post type options that this theme provides an archive for
				'options' => array(
					'post'			=> _x( 'Blog', 'archives widget', 'uplifted' ),
					'ctc_sermon'	=> _x( 'Sermons', 'archives widget', 'uplifted' ),
				),
			),
		),
	) );
	...
	add_theme_support( 'ctfw-widget-slide', array(
		...
		'field_overrides' => array(
			'image_id' => array( // tell user image is required with this theme
				'after_name' => _x( '(Required)', 'slide widget image', 'uplifted' ),
				'desc' => sprintf( __( 'Image cropped to %s.', 'uplifted' ), ctfw_image_size_dimensions( 'uplifted-slide' ) ),
			),
...
add_action( 'after_setup_theme', 'uplifted_add_theme_support_framework_widgets' );
