 ...
 */
function uplifted_banner_related_pages() {
	...
	return apply_filters( 'uplifted_banner_pages', $pages_query->posts );
...
function uplifted_banner_data() {
	...
	// Priority 4: Not on page with featured image -- use related page's featured image
	if ( empty( $data['page'] ) ) {
		...
		if ( $page_templates ) {
			...
			$banner_pages = uplifted_banner_related_pages();
	...
	// Return filtered
	return apply_filters( 'uplifted_banner_data', $data );
