...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( 'resurrect-entry-full resurrect-event-full' ); ?>>
		...
		<?php if ( $directions_url ) : ?>
			<div class="resurrect-event-full-direction">
				<a href="<?php echo esc_url( $directions_url ); ?>" target="_blank" class="resurrect-button">
					<span class="<?php resurrect_icon_class( 'event-directions' ); ?> resurrect-button-icon"></span>
					<?php _ex( 'Get Directions', 'event', 'resurrect' ); ?>
				</a>
			</div>
		<?php endif; ?>
