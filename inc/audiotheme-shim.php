<?php
/**
 * A drop-in for themes to help prevent them from completely breaking in case
 * the AudioTheme framework plugin is deactivated.
 *
 * @package AudioTheme
 * @subpackage Shim
 * @version 1.0.0
...
if ( ! is_audiotheme_active() ) :
	 ...
	 * @since 1.0.0
	 */
	function audiotheme_framework_required_notice() {
		...
		<div class="error">
			<p>
				<?php
				_e( 'The AudioTheme Framework is required to be installed and activated for this theme to display properly.', 'americanaura' );
				...
				if ( 0 === validate_plugin( 'audiotheme/audiotheme.php' ) ) {
					...
					echo ' <a href="' . esc_url( $activate_url ) . '"><strong>' . __( 'Activate now.', 'americanaura' ) . '</strong></a>';
				} else {
					echo ' <a href="http://audiotheme.com/"><strong>' . __( 'Find out more.', 'americanaura' ) . '</strong></a>';
				}
				?>
			</p>
		</div>
	...
	add_action( 'admin_notices', 'audiotheme_framework_required_notice' );
	 ...
	 *
	 * Function called to get a theme option. The returned value defaults to
	 * false unless a default is passed.
	 *
	 * Note that this function footprint is slightly different than
	 * get_audiotheme_option(). While working in themes, the $option_name
	 * shouldn't necessarily need to be known or required, so it should be
	 * slightly easier to use while in a theme.
	 *
	 * @since 1.0.0
	...
	/**
	 * Check if the theme supports theme options and return registered
	 * arguments with supplied defaults.
	 *
	...
	function get_audiotheme_theme_options_support( $var = null ) {
		if ( $support = get_theme_support( 'audiotheme-theme-options' ) ) {
			...
			$args = array(
				...
				'menu_title'  => __( 'Theme Options', 'americanaura' ),
			);
